
from collections import OrderedDict
from all import setting
import requests
import simplejson as json
import xlrd


# region Data into db
def write_facility():
    auth_data = {"code": "666666", "mobile": "01117179264"}
    auth = requests.post(f"{setting.base_api_uri}/authenticates", json=auth_data)

    get_id = requests.post(f"{setting.base_api_uri}/authenticates", json=auth_data)
    get_id_json = get_id.json()
    auth_id = get_id_json['code']
    print(auth_id)
    workbook = xlrd.open_workbook(setting.path_excel)
    sheet = workbook.sheet_by_name('Facilities')
    print(sheet)
    set_enabled = True
    for row in range(1, sheet.nrows):
        data = OrderedDict()
        row_values = sheet.row_values(row)
        data['slug'] = row_values[0]
        data['title'] = row_values[1].strip()
        data['is_enabled'] = set_enabled
        data['icon'] = row_values[2]

        res = requests.post(f"{setting.base_api_uri}/facilities", json=data, headers={'Authorization': f"Bearer {auth_id}"})
        # print(res)
        print(res.json)

    return "down"


# endregion

# write_facility(setting.path_excel)
